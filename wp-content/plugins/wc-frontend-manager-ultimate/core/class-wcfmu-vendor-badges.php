...
class WCFMu_Vendor_Badges
{
    ...
    public function __construct()
    {
        ...
        if ($WCFMu->is_marketplace == 'wcmarketplace') {
			...
			// 不在商品列表中显示多余的徽章
			add_action('custom_wcfmmp_store_mobile_badges', [ &$this, 'custom_after_wcfmmp_sold_by_label_product_page' ], 15);
    ...
    function wcfmu_membership_badges_manage($membership_id)
    {
        ...
        if (empty($this->wcfm_vendor_badges_options)) {
            // translators: 1) knowledgebase url
                    ...
                    $WCFM->wcfm_fields->wcfm_generate_form_field(
                        [
                            'wcfm_membership_badges_'.$badge_key => [
                                'label'       => '<img alt="badge" src="'.wcfm_get_attachment_url($wcfm_vendor_badges_option['badge_icon']).'" style="width: 32px; margin-right: 5px; display: inline-block;">'.$wcfm_vendor_badges_option['badge_name'],
    ...
    public function show_wcfm_vendor_badges($vendor_id=0, $is_large=false)
    {
        ...
        if ($vendor_id) {
            ...
            echo '<div class="wcfm_vendor_badges 1">';
            ...
            if (! empty($wcfm_vendor_badges)) {
                foreach ($this->wcfm_vendor_badges_options as $badge_key => $wcfm_vendor_badges_option) {
                    if (isset($wcfm_vendor_badges_option['is_active']) && ! empty($wcfm_vendor_badges_option['badge_name']) && isset($wcfm_vendor_badges[$badge_key])) {
                        echo '<div class="'.$badge_classses.' text_tip"  data-tip="'.$wcfm_vendor_badges_option['badge_name'].'"><img alt="badge" src="'.wcfm_get_attachment_url($wcfm_vendor_badges_option['badge_icon']).'" /></div>';
                    }
                }
            }
	...
	public function custom_show_wcfm_vendor_badges($vendor_id=0, $is_large=false)
	    {
	        global $WCFM, $WCFMu;
	        ...
	        if ($vendor_id) {
	            ...
	            if (! empty($wcfm_vendor_badges)) {
	                foreach ($this->wcfm_vendor_badges_options as $badge_key => $wcfm_vendor_badges_option) {
	                    if (isset($wcfm_vendor_badges_option['is_active']) && ! empty($wcfm_vendor_badges_option['badge_name']) && isset($wcfm_vendor_badges[$badge_key])) {
	                        echo '<div class="'.$badge_classses.' text_tip"  data-tip="'.$wcfm_vendor_badges_option['badge_name'].'"><img class="1" alt="badge" src="'.wcfm_get_attachment_url($wcfm_vendor_badges_option['badge_icon']).'" /></div>';
	                    }
	                }
	            }
                ...
                if (! empty($wcfm_membership_badges)) {
                    ...
                    foreach ($wcfm_vendor_badges_options as $badge_key => $wcfm_vendor_badges_option) {
                        if (isset($wcfm_vendor_badges_option['is_active']) && ! empty($wcfm_vendor_badges_option['badge_name']) && isset($wcfm_membership_badges[$badge_key])) {
                            echo '<div class="wcfm_vendor_badge wcfm_vendor_badge_large text_tip"  data-tip="'.$wcfm_vendor_badges_option['badge_name'].'"><img alt="badges" src="'.wcfm_get_attachment_url($wcfm_vendor_badges_option['badge_icon']).'" /></div>';
                        }
                    }
    ...
    function after_wcfm_dashboard_user($vendor_id)
    {
        ...
        if (! empty($wcfm_vendor_badges)) {
            foreach ($this->wcfm_vendor_badges_options as $badge_key => $wcfm_vendor_badges_option) {
                if (isset($wcfm_vendor_badges_option['is_active']) && ! empty($wcfm_vendor_badges_option['badge_name']) && isset($wcfm_vendor_badges[$badge_key])) {
                    echo '<img alt="badge" class="wcfm_vendor_badge text_tip"  data-tip="'.$wcfm_vendor_badges_option['badge_name'].'" src="'.wcfm_get_attachment_url($wcfm_vendor_badges_option['badge_icon']).'" />';
                }
				// break;
            }
        }
    ...
    function wcfm_dokan_product_seller_tab($val)
    {
        ...
        if ($vendor_id) {
            ...
            if (! empty($wcfm_vendor_badges)) {
                foreach ($this->wcfm_vendor_badges_options as $badge_key => $wcfm_vendor_badges_option) {
                    if (isset($wcfm_vendor_badges_option['is_active']) && ! empty($wcfm_vendor_badges_option['badge_name']) && isset($wcfm_vendor_badges[$badge_key])) {
                        $author->display_name .= '<div class="wcfm_vendor_badge wcfm_vendor_badge_large text_tip"  data-tip="'.$wcfm_vendor_badges_option['badge_name'].'"><img alt="badge" src="'.wcfm_get_attachment_url($wcfm_vendor_badges_option['badge_icon']).'" /></div>';
                    }
                }
            }
        ...
        if ($vendor_id) {
            ...
            if ($vendor_admin_id) {
                ...
                if (! empty($wcfm_vendor_badges)) {
                    foreach ($this->wcfm_vendor_badges_options as $badge_key => $wcfm_vendor_badges_option) {
                        if (isset($wcfm_vendor_badges_option['is_active']) && ! empty($wcfm_vendor_badges_option['badge_name']) && isset($wcfm_vendor_badges[$badge_key])) {
                            $name .= '<div class="wcfm_vendor_badge text_tip"  data-tip="'.$wcfm_vendor_badges_option['badge_name'].'"><img alt="badge" src="'.wcfm_get_attachment_url($wcfm_vendor_badges_option['badge_icon']).'" /></div>';
                        }
						break;
                    }
                }
	...
	function custom_after_wcfmmp_sold_by_label_product_page($vendor_id)
    {
        global $WCFM, $WCFMu;
        if ($vendor_id) {
            if (apply_filters('wcfm_is_allow_badges_in_loop', true)) {
                $this->custom_show_wcfm_vendor_badges($vendor_id);
            }
        }
    ...
    function show_conditional_badges($vendor_id, $badge_classses)
    {
        ...
        if (! empty($wcfm_conditional_badges_rules)) {
            foreach ($wcfm_conditional_badges_rules as $rule) {
                ...
                if ($show_badge) {
                    echo '<div class="'.$badge_classses.' text_tip"  data-tip="'.$rule['badge_name'].'"><img alt="badge" src="'.wcfm_get_attachment_url($rule['badge_icon']).'" /></div>';
                }
            }//end foreach
        }//end if
