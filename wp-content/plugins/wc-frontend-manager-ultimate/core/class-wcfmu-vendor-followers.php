...
class WCFMu_Vendor_Followers
{
    ...
    public function __construct()
    {
        ...
        $this->wcfm_followers_options = get_option('wcfm_followers_options', []);

        $wcfm_myac_modified_endpoints             = wcfm_get_option('wcfm_myac_endpoints', []);
        ...
        add_filter('wcfm_query_vars', [ &$this, 'wcfm_followers_query_vars' ], 20);
        add_filter('wcfm_endpoint_title', [ &$this, 'wcfm_followers_endpoint_title' ], 20, 2);
        add_action('init', [ &$this, 'wcfm_followers_init' ], 20);

        // Followers Endpoint Edit
        add_filter('wcfm_endpoints_slug', [ $this, 'followers_wcfm_endpoints_slug' ]);

        // Followers menu on WCfM dashboard
        if (apply_filters('wcfm_is_allow_followers', true)) {
            add_filter('wcfm_menus', [ &$this, 'wcfm_followers_menus' ], 30);
        }

        // Followers Load Scripts
        add_action('wcfm_load_scripts', [ &$this, 'load_scripts' ], 30);
        add_action('after_wcfm_load_scripts', [ &$this, 'load_scripts' ], 30);

        // Followers Load Styles
        add_action('wcfm_load_styles', [ &$this, 'load_styles' ], 30);
        add_action('after_wcfm_load_styles', [ &$this, 'load_styles' ], 30);
        ...
        add_action('wp_ajax_nopriv_wcfm_ajax_controller', [ &$this, 'ajax_controller' ]);

        // My Account Followers End Point
        add_action('init', [ &$this, 'wcfm_followers_my_account_endpoints' ]);

        // My Account Followers Query Vars
        add_filter('query_vars', [ &$this, 'wcfm_followers_my_account_query_vars' ], 0);

        // My Account Followers Rule Flush
        register_activation_hook($WCFMu->file, [ &$this, 'wcfm_followers_my_account_flush_rewrite_rules' ]);
        register_deactivation_hook($WCFMu->file, [ &$this, 'wcfm_followers_my_account_flush_rewrite_rules' ]);

        // My Account Followers Menu
        add_filter('woocommerce_account_menu_items', [ &$this, 'wcfm_followers_my_account_menu_items' ], 190);

        // My Account Followers End Point Title
        add_filter('the_title', [ &$this, 'wcfm_followers_my_account_endpoint_title' ]);
        ...
        add_action('woocommerce_account_'.$this->wcfm_myaccount_followings_endpoint.'_endpoint', [ &$this, 'wcfm_followings_my_account_endpoint_content' ]);

        // Followers notification on new product submited
        if (apply_filters('wcfm_is_allow_followers_new_product_notification', true)) {
            add_action('wcfm_after_new_product_by_vendor', [ &$this, 'wcfmu_vendors_followers_notify_on_new_product' ], 10, 2);
        }
        ...
        add_action('wp_ajax_wcfmu_vendors_followers_list', [ &$this, 'wcfmu_vendors_followers_list' ]);

        // Update Vendor Followers
        add_action('wp_ajax_wcfmu_vendors_followers_update', [ &$this, 'wcfmu_vendors_followers_update' ]);

        // Delete Followers
        add_action('wp_ajax_delete_wcfm_followers', [ &$this, 'wcfmu_vendors_followers_delete' ]);

        // Delete Followings
        add_action('wp_ajax_delete_wcfm_followings', [ &$this, 'wcfmu_vendors_followings_delete' ]);

        // Followers direct message type
        add_filter('wcfm_message_types', [ &$this, 'wcfm_followers_message_types' ], 45);

        // enqueue scripts
        add_action('wp_enqueue_scripts', [ &$this, 'wcfm_followers_scripts' ]);
        // enqueue styles
        add_action('wp_enqueue_scripts', [ &$this, 'wcfm_followers_styles' ]);

        if ($WCFMu->is_marketplace == 'wcmarketplace') {
            add_action('before_wcmp_vendor_information', [ &$this, 'before_wcmp_vendor_information' ], 16);
            ...
            if (WCFM_Dependencies::wcvpro_plugin_active_check()) {
                add_action('wcv_after_vendor_store_title', [ &$this, 'after_wcv_pro_store_header' ], 16);
            } else {
                add_action('wcv_after_main_header', [ &$this, 'after_wcv_store_header' ], 16);
                add_action('wcv_after_mini_header', [ &$this, 'after_wcv_store_header' ], 16);
            }
            ...
            add_action('dokan_store_header_info_fields', [ &$this, 'after_dokan_store_header' ], 16);
            ...
            add_action('wcfmmp_store_follow_me', [ &$this, 'wcfmmp_store_follow_me' ], 16);
        }//end if
    ...
    }//end wcfm_followers_endpoint_title()
    ...
    }//end followers_wcfm_endpoints_slug()
    ...
    function wcfm_followers_menus($menus)
    {
        ...
        $menus = (array_slice($menus, 0, 3, true) + [
            'wcfm-followers' => [
                'label'    => __('Followers', 'wc-frontend-manager-ultimate'),
    ...
    public function load_scripts($end_point)
    {
        ...
        switch ($end_point) {
            case 'wcfm-followers':
                ...
                wp_enqueue_script('wcfmu_followers_js', $WCFMu->library->js_lib_url.'followers/wcfmu-script-followers.js', [ 'jquery' ], $WCFMu->version, true);
            ...
            case 'wcfm-followings':
                wp_enqueue_script('wcfmu_followings_js', $WCFMu->library->js_lib_url.'followers/wcfmu-script-followings.js', [ 'jquery' ], $WCFMu->version, true);
                // Localized Script
    ...
    public function load_styles($end_point)
    {
        ...
        switch ($end_point) {
            case 'wcfm-followers':
                wp_enqueue_style('wcfmu_followers_css', $WCFMu->library->css_lib_url.'followers/wcfmu-style-followers.css', [], $WCFMu->version);
                break;

            case 'wcfm-followings':
                wp_enqueue_style('wcfmu_followings_css', $WCFMu->library->css_lib_url.'followers/wcfmu-style-followings.css', [], $WCFMu->version);
                break;
        }
    ...
    }//end wcfm_followers_my_account_endpoints()
    ...
    }//end wcfm_followers_my_account_query_vars()
    ...
    }//end wcfm_followers_my_account_flush_rewrite_rules()
    ...
    }//end wcfm_followers_my_account_menu_items()
    ...
    function wcfm_followers_my_account_endpoint_title($title)
    {
        ...
        if ($is_endpoint && ! is_admin() && is_main_query() && in_the_loop() && is_account_page()) {
            ...
            remove_filter('the_title', [ $this, 'wcfm_followers_my_account_endpoint_title' ]);
        }

        return $title;

    }//end wcfm_followers_my_account_endpoint_title()
    ...
    }//end wcfm_followings_my_account_endpoint_content()
            ...
            foreach ($vendor_followers as $vendor_follower) {
                if ($vendor_follower->follower_email) {
                    ...
                    if( apply_filters( 'wcfm_allow_wpml_email_translation', true ) ) {
                        do_action('wpml_switch_language_for_email', $vendor_follower->follower_email);
                    }

                    $notificaton_mail_subject = '{site_name}: '.__('New Product', 'wc-frontend-manager-ultimate').' - {product_title}';
                    $notification_mail_body   = '<br/>'.__('Hi', 'wc-frontend-manager').' {follower_name}'.',<br/><br/>'.__('A new product submitted by ', 'wc-frontend-manager-ultimate').' <b>{vendor_store}</b>'.'<br/><br/>'.
                    // translators: 1) begin url 2) end url
                    sprintf(__('Check the product %1$s{product_title}%2$s.', 'wc-frontend-manager-ultimate'), '<a href="{product_url}">', '</a>')
                    .'<br /><br/>'.__('Thank You', 'wc-frontend-manager').'<br/><br/>';
    ...
    }//end wcfmu_vendors_followers_notify_on_new_product()
    ...
    function wcfmu_followers_settings($wcfm_options)
    {
        ...
        if (! $followers_icon) {
            $followers_icon = $WCFMu->plugin_url.'assets/images/verification_badge.png';
        }
        ...
        <div class="wcfm-container">
            <div id="wcfm_settings_form_followers_expander" class="wcfm-content">
                ...
                $WCFM->wcfm_fields->wcfm_generate_form_field(
                    apply_filters(
                        'wcfmu_settings_fields_followers_general',
                        [
                            'followers_icon' => [
                                'label'       => __('Followers Icon', 'wc-frontend-manager-ultimate'),
                                'name'        => 'wcfm_followers_options[followers_icon]',
                                'type'        => 'upload',
                                'class'       => 'wcfm_ele',
                                'prwidth'     => 64,
                                'label_class' => 'wcfm_title wcfm_ele',
                                'hints'       => __('Upload badge image 32x32 size for best view.', ''),
                                'value'       => $followers_icon,
                            ],
                        ]
                    )
                );
    ...
    }//end wcfmu_followers_settings_update()
    ...
    }//end wcfmu_vendors_followers_list()
    ...
    }//end wcfmu_vendors_followers_update()
    ...
    }//end wcfmu_vendors_followers_delete()
    ...
    }//end wcfmu_vendors_followings_delete()
    ...
    }//end wcfm_followers_message_types()
        ...
        if (is_account_page()) {
            if (is_user_logged_in()) {
                if (isset($wp->query_vars[$this->wcfm_myaccount_followings_endpoint])) {
                    ...
                    wp_enqueue_script('wcfmu_my_account_followings_js', $WCFMu->library->js_lib_url.'followers/wcfmu-script-my-account-followings.js', [ 'jquery' ], $WCFMu->version, true);
    ...
    function wcfm_followers_styles()
    {
        ...
        if (is_account_page()) {
            if (is_user_logged_in()) {
                // if( isset( $wp->query_vars[$this->wcfm_myaccount_followings_endpoint] ) ) {
                    wp_enqueue_style('wcfmu_my_account_support_manage_css', $WCFMu->library->css_lib_url.'followers/wcfmu-style-my-account-followings.css', [], $WCFMu->version);
                // }
            }
        }

    }//end wcfm_followers_styles()
    ...
    }//end show_wcfm_vendor_followers()
    ...
    function wcfmmp_store_follow_me($vendor_id)
    {
        ...
        if (is_user_logged_in()) {
            ...
            if ($user_id && ! $is_following) {
                ?>
                <div class="lft bd_icon_box"><a style="border:0.1px solid black;" id="wcfm_follow_now" title="<?php _e('Click to Follow', 'wc-frontend-manager-ultimate'); ?>" data-count="<?php echo $followers; ?>"  data-vendor_id="<?php echo $vendor_id; ?>" data-user_id="<?php echo $user_id; ?>" href="#" class="follow"><i class="wcfmfa fa-heart"></i>&nbsp;<span style="display:inline-block"><?php echo apply_filters('wcfm_store_follow_button_label', __('Follow', 'wc-frontend-manager-ultimate'));echo "(".$followers.")" ?></span></a></div>
                <script>
                    jQuery(document).ready(function($) {
                        $('#wcfm_follow_now').click(function(event) {
                            event.preventDefault();
                ...
                ?>
                <div class="lft bd_icon_box"><a style="border:1px solid black;" id="wcfm_follow_delete" title="<?php _e('Click to Un-follow', 'wc-frontend-manager-ultimate'); ?>" class="follow wcfm_followings_delete"  data-followersid="<?php echo $user_id; ?>" data-userid="<?php echo $vendor_id; ?>" href="#"><i class="wcfmfa fa-heart" style="color: black;font-weight:600!important;"></i>&nbsp;<span style="display:inline-block"><?php _e('Followed', 'wc-frontend-manager-ultimate');echo "(".$followers.")"   ?></span></a></div>
                <?php
                wp_enqueue_script('wcfmu_my_account_followings_js', $WCFMu->library->js_lib_url.'followers/wcfmu-script-my-account-followings.js', [ 'jquery' ], $WCFMu->version, true);
                $wcfm_dashboard_messages = get_wcfm_dashboard_messages();
                wp_localize_script('wcfmu_my_account_followings_js', 'wcfm_dashboard_messages', $wcfm_dashboard_messages);
            ...
            ?>
            <div class="lft bd_icon_box"><a style="border:1px solid black;" id="wcfm_follow_now" title="<?php _e('Click to Follow', 'wc-frontend-manager-ultimate'); ?>"  href="#" class="follow wcfm_login_popup" ><i class="wcfmfa fa-heart"></i>&nbsp;<span style="display:inline-block"><?php echo apply_filters('wcfm_store_follow_button_label', __('Follow', 'wc-frontend-manager-ultimate'));echo "(".$followers.")"  ?></span></a></div>
            <?php
        }//end if

    }//end wcfmmp_store_follow_me()
